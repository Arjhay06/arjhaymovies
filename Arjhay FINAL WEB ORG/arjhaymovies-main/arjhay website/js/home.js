...
async function init() {
  // Attach global search functions
...
function changeServer() {
  ...
  switch (server) {
    ...
    case '2embed':
      ...
      case '2anime.xyz': {
        // turn the title into a URL‑slug: lowercase, alphanumeric + hyphens
...
// Load seasons and save on change
...
// Display episodes in scrollable list, save and highlight
async function displayEpisodes(item) {
  ...
  // clear old list and hide pagination
  list.innerHTML = '';
  const pagination = document.getElementById('episode-pagination');
  if (pagination) pagination.style.display = 'none';

  // create scrollable container via CSS
...
async function searchTMDB() {
  ...
  if (!q) return;
  const data = await (await fetch(`${BASE_URL}/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(q)}`)).json();
